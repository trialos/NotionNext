...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    const normalizedSlug = fullSlug?.trim()
    return (
      // 1. 完整 slug 匹配
      props.allPages.find(p => p && !p.type?.includes('Menu') && p.slug === fullSlug) ||
      // 1b. 去除空格后匹配（防止 Notion Slug 字段首尾有空格）
      props.allPages.find(p => p && !p.type?.includes('Menu') && p.slug?.trim() === normalizedSlug) ||
      // 2. UUID 匹配
  ...
  // 诊断日志：文章未找到时输出 allPages 中的 slug 样本，方便排查
  if (!post) {
    const sample = props.allPages?.slice(0, 15).map(p => p?.slug).filter(Boolean)
    console.warn(`[${taskId}] ⚠️ 文章未找到 fullSlug="${fullSlug}"，allPages slug 示例:`, sample)
  }
